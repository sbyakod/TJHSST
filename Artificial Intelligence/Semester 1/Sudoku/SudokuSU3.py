...
def getLeast(dict4):
    ...
    for each in dict4:
        if dict4[each] != None:
            if len(dict4[each]) != 0:
                if len(dict4[each]) < least:
                    least = len(dict4[each])
                    index = each
                    final = dict4[each]
    return index, final
...
def bruteForce(pzl,index,pdList,count,dict4):
    # print("".join(pzl))
    # print(dict4)
    # if count == 0:
    ...
    for y in set(poss):
        ...
        dict4[position].pop()
        subbf = pzl[:]
        dict2 = {i:set(dict4[i]) for i in dict4}
        dict2.pop(position)
        for each in dictionary3[position]:
            if each in dict2:
                if y in dict2[each]:
                    dict2[each].remove(y)
                    # if(len(dict2[each]) == 0):
        ...
        bf = bruteForce(subbf,index,pdList,count,dict2)
        if(bf != ""):
            return bf
    return ""
...
for each in lines:
    ...
    for z in range(0,81):
        if(each[z] == "."):
            pdList.append(z)
    dict4 = getLeastPos(each,pdList)
    x = bruteForce(each,pdList[0],pdList,0,dict4)
    final = ""
    for each in x:
        final += str(each)
    print(final)
