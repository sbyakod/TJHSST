        ...
        for i in range(len(path)):
            for j in range(i+1, len(path)):
                ...
                if distances[seg1[0]][seg1[1]] + distances[seg2[0]][seg2[1]] > distances[seg1[0]][seg2[0]] + distances[seg1[1]][seg2[1]]:
                    updated = path[:i+1] + path[i+1:j+1][::-1] + path[j+1:]
                    check = True
                if check: path = updated[:]
    return updated
...
def pt1():
    ...
    #for vertex in vertices:
         #canvas1.create_oval(coordinates[vertex][0]-5, coordinates[vertex][1]+5,coordinates[vertex][0]+5, coordinates[vertex][1]-5, fill="black",width=.25)
    for i in range(len(vertices)):
        path.append(str(i))
        distance += distances[vertices[i]][vertices[(i+1)%numVertices]]
        canvas1.create_line(coordinates[vertices[i]][0],coordinates[vertices[i]][1], coordinates[vertices[(i+1)%numVertices]][0], coordinates[vertices[(i+1)%numVertices]][1], fill="blue",width=.25)
...
def pt2():
    ...
    path = uncross(path)
    #for vertex in vertices:
         #canvas2.create_oval(coordinates[vertex][0]-5, coordinates[vertex][1]+5,coordinates[vertex][0]+5, coordinates[vertex][1]-5, fill="black",width=.25)
    for i in range(len(path)):
        distance += distances[vertices[path[i]]][vertices[path[(i+1)%numVertices]]]
        canvas2.create_line(coordinates[vertices[path[i]]][0],coordinates[vertices[path[i]]][1], coordinates[vertices[path[(i+1)%numVertices]]][0], coordinates[vertices[path[(i+1)%numVertices]]][1], fill="blue",width=.25)
...
def pt3():
    global permwidth
    if file != 'KAD.txt': permwidth = 6
    distance, olddist = 0, sys.maxsize
    path = [i for i in range(numVertices)]
    canvas3 = Canvas(root3, width=600,height=600, background="white")
    canvas3.pack()
    path = uncross(path)
    distance = getDistance(path)
    while(distance < olddist):
        path = uncross(permute(path))
        olddist = distance
        distance = getDistance(path)
    #for vertex in vertices:
         #canvas3.create_oval(coordinates[vertex][0]-5, coordinates[vertex][1]+5,coordinates[vertex][0]+5, coordinates[vertex][1]-5, fill="black",width=.25)
    for i in range(len(path)):
        canvas3.create_line(coordinates[vertices[path[i]]][0],coordinates[vertices[path[i]]][1], coordinates[vertices[path[(i+1)%numVertices]]][0], coordinates[vertices[path[(i+1)%numVertices]]][1], fill="blue",width=.25)
    path = [str(i) for i in path]
